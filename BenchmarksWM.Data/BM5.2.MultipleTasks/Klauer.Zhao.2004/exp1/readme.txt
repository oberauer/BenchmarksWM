...
For the movemt task, the response is correct if stimulus ID = response ID. For the colors task, the response with ID 1 is correct if stimulus ID < 8; and the response with ID 2 is correct if stimulus ID>8.

The Pascal program auswert.pas takes these as input and produces the output files ergebnis and zweitno.

The file "ergebnis" is included in SPSS format with variable labels as the file "data1-20.sav". It contains percentages of correct responses in the
